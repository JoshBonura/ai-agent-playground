...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const hardLogoutLocal = () => {
    try { localStorage.removeItem("profile_email"); } catch {}
    setUser(null);
  };
  ...
  const refreshMe = async () => {
    try {
      ...
      setUser(me || null);
      if (me?.email) { try { localStorage.setItem("profile_email", me.email); } catch {} }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (Date.now() - justLoggedOutAt < 800) return; // tiny debounce after logout
        const me = await getJSON<LocalUser>("/auth/me");
  ...
  const ctx = useMemo<Ctx>(() => ({ user, loading, refreshMe, logout }), [user, loading]);

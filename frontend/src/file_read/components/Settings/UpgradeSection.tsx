...
export default function UpgradeSection({ userEmail }: Props) {
  ...
  const isPro = useMemo(() => {
    ...
    const licActive = Boolean(license?.valid) && (license?.plan || "pro").toLowerCase() !== "free";
  ...
  async function handleUpgrade() {
    if (!userEmail) { setErr("Please sign in to upgrade."); return; }
    try {
      setErr(null);
      const priceId = (import.meta.env.VITE_STRIPE_PRICE_PRO_MONTHLY as string | undefined) || undefined;
  ...
  async function handleManage() {
    if (!userEmail) { setErr("Please sign in to manage billing."); return; }
    try {
      setErr(null);
...
function PlanCard({
  title, price, subtitle, features, ctaLabel, ctaDisabled, onClick, highlight,
}:{

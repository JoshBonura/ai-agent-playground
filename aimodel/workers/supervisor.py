...
# keep the repo-local path (for dev)
_RUNTIMES_DIR = Path(".runtime") / "venvs"
...
def _app_data_dir() -> Path:
    ...
    return Path(os.getenv("XDG_DATA_HOME", Path.home() / ".local" / "share")) / "LocalAI"

_RUNTIME_DIR_CANDIDATES = [
    _RUNTIMES_DIR,                            # repo-local
    _app_data_dir() / ".runtime" / "venvs",   # shared app data (runtime_manager)
]

def _resolve_repo_root() -> Path:
    """
    Return a path that contains the top-level `aimodel` package.
    - Dev: repo root (…/ai-agent-playground)
    - Packaged: Electron resources/_internal
    """
    # Packaged (PyInstaller) — the backend is an .exe inside Electron's resources dir
    if getattr(sys, "frozen", False):
        # sys.executable = ...\resources\localmind-backend.exe
        res_dir = Path(sys.executable).parent
        internal = res_dir / "_internal"
        # We ship aimodel at resources/_internal/aimodel
        if (internal / "aimodel").exists():
            return internal
        # Fallback: sometimes PyInstaller extracts to a temp tree; prefer that if present
        me = Path(__file__).resolve()
        cand = me
        # Walk up a few levels to find an _internal/aimodel sibling
        for _ in range(6):
            if ((cand.parent / "_internal" / "aimodel").exists()):
                return (cand.parent / "_internal")
            cand = cand.parent
    # Dev workspace:  .../aimodel/workers/supervisor.py -> go up three to repo root
    # adjust if your mono-repo depth differs
    return Path(__file__).resolve().parents[3]


def _resolve_runtime_python() -> Optional[str]:
    # 1) explicit env override
    for k in ("LM_RUNTIME_PYTHON", "LOCALMIND_RUNTIME_PY"):
        v = os.getenv(k)
        if v and Path(v).exists():
            return v

    # 2) any known active.json
    import json
    for root in _RUNTIME_DIR_CANDIDATES:
        try:
            p = root / "active.json"
            if p.exists():
    ...
    # 3) fallback (let worker inherit current interpreter)
    return None
...
class ModelWorkerSupervisor:
    ...
    async def spawn_worker(self, model_path: str, llama_kwargs: dict | None = None) -> WorkerInfo:
        ...
        app_module = "aimodel.workers.worker_entry:app"
        log.info(
            f"[workers] spawning {wid} on {host_bind}:{port} for {model_path} "
            f"(accel={pref.accel}, n_gpu_layers={pref.n_gpu_layers})"
        )

        py_exec = _resolve_runtime_python() or sys.executable

        # In frozen/packaged mode, NEVER use the PyInstaller EXE to run "-m uvicorn".
        # Prefer the provisioned runtime python if available; if not, fall back to sys.executable
        # but emit a big red flag so we can see it immediately.
        if getattr(sys, "frozen", False):
            venv_py = _resolve_runtime_python()
            if venv_py and Path(venv_py).exists():
                py_exec = venv_py
                log.info("[workers.spawn] packaged: using venv python: %s", py_exec)
            else:
                # Last resort – we will still try, but this will likely re-exec the EXE and re-boot the backend.
                log.warning("[workers.spawn] packaged: NO venv python found; falling back to sys.executable=%s (this can re-spawn the full backend!)",
                            sys.executable)
                py_exec = sys.executable

        cmd = [
            py_exec, "-m", "uvicorn", app_module,
        ...
        # After: env.update(env_patch)
        if pref.accel and pref.accel != "auto":
            env["LLAMA_ACCEL"] = pref.accel
            if pref.accel == "cpu":
                # ensure no accidental GPU use
                env["CUDA_VISIBLE_DEVICES"] = "-1"
                env.pop("GGML_CUDA", None)
                # if user didn't explicitly request GPU layers, force 0
                if int(cleaned.get("n_gpu_layers") or 0) > 0 and "n_gpu_layers" not in (llama_kwargs or {}):
        ...
        log.info(
    "[workers.spawn.cmd] py=%s argv=%s",
    py_exec,
    " ".join([
        py_exec, "-m", "uvicorn", app_module,
        "--host", host_bind, "--port", str(port), "--log-level", "info"
    ])
        )
        log.info(
            "[workers.spawn.env] MODEL_PATH=%s LLAMA_ACCEL=%s PYTHONPATH(head)=%s",
            model_path, env.get("LLAMA_ACCEL"), (env.get("PYTHONPATH","").split(os.pathsep)[0] if env.get("PYTHONPATH") else "")
        )
        log.info(
            "[workers.spawn.env] WORKER_HOST=%s WORKER_PORT=%s",
            env.get("WORKER_HOST"), env.get("WORKER_PORT")
        )

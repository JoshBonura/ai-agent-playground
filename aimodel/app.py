...
# 🔥 runtime warm-provision (optional, best-effort)
try:
    from ext.provision import warm_provision_all_possible
except Exception:
    warm_provision_all_possible = None  # type: ignore[assignment]
...
# Mount runtime API from ext/
try:
    from ext.runtime_api import router as runtime_router
    app.include_router(runtime_router)  # exposes /api/runtime/* on the main app
    log.info("[runtime] mounted ext.runtime_api")
except Exception as e:
    log.warning("[runtime] ext.runtime_api not mounted: %r", e)

    

# ---------- Serve built frontend (Vite 'dist') ----------
ENV_FRONTEND = os.getenv("LM_FRONTEND_DIST", "").strip()
if ENV_FRONTEND:
    FRONTEND_DIR = Path(ENV_FRONTEND)
else:
...
async def _startup():
    ...
    log.info("removed the snapshot poller")
    # warm-provision ...
    if warm_provision_all_possible:
        try:
            loop = asyncio.get_running_loop()
            await loop.run_in_executor(None, warm_provision_all_possible)
            log.info("[runtime] warm provision complete (best-effort)")
        except Exception as e:
            log.warning("[runtime] warm provision skipped: %r", e)

    # auto-pick backend (CUDA/Metal → CPU)
    try:
        from ext.worker import switch_auto
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, switch_auto)
        log.info("[runtime] auto-selected backend")
    except Exception as e:
        log.warning("[runtime] auto backend selection failed: %r", e)

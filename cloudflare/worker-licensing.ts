...
<div class="card">
  ...
  <form method="POST" action="/api/checkout/session">
    ...
    if (url.pathname === "/success") {
      ...
      const masked = lic ? lic.slice(0, 16) + "… (copy below)" : "(not ready yet — webhook may still be processing; refresh in a few seconds)";
...
`);
    ...
    if (request.method === "POST" && url.pathname === "/api/checkout/session") {
      ...
      return withCORS(html(`<!doctype html><meta http-equiv="refresh" content="0;url=${body.url}">Redirecting…`), allowOrigin(request));
    }
